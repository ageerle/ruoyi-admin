...
import type { ExtractionResult, GraphRetrievalResult } from '#/api/operator/graph/model';
          ...
          <Card title="实体抽取测试" size="small">
            <Space direction="vertical" style="width: 100%" :size="16">
              ...
              <div v-if="extractionResult">
                <Alert message="抽取结果" type="success" style="margin-bottom: 16px" />
          ...
          <Card title="文本入库测试" size="small">
            <Space direction="vertical" style="width: 100%" :size="16">
              ...
              <div v-if="ingestResult">
                <Alert message="入库成功" type="success" style="margin-bottom: 16px" />
              ...
              <Col :span="16">
                <Spin :spinning="retrieveLoading" tip="检索中...">
                  <div v-if="retrievalResult" style="min-height: 400px">
                    <Alert message="检索结果" type="success" style="margin-bottom: 16px" />
                    
                    <div style="margin-bottom: 16px">
                      <div style="font-weight: 500; margin-bottom: 8px">增强内容</div>
                      <Card size="small">
                        <pre style="margin: 0; white-space: pre-wrap">{{ retrievalResult.content }}</pre>
                      </Card>
                    </div>
                    
                    <div style="margin-bottom: 16px">
                      <div style="font-weight: 500; margin-bottom: 8px">
                        相关实体 ({{ retrievalResult.relevantEntities?.length || 0 }})
                      </div>
                      <Space wrap>
                        <Tag
                          v-for="(entity, index) in retrievalResult.relevantEntities"
                          :key="index"
                          color="blue"
                        >
                    ...
                    <div>
                      <div style="font-weight: 500; margin-bottom: 8px">
                        相关关系 ({{ retrievalResult.relevantRelations?.length || 0 }})
                      </div>
                      <div
                        v-for="(relation, index) in retrievalResult.relevantRelations"
                        :key="index"
                        style="margin-bottom: 8px"
                      >
                  ...
                  <div v-else style="min-height: 400px; display: flex; align-items: center; justify-content: center; color: #999">
                    暂无检索结果
                  </div>
                </Spin>
              </Col>
