...
import type { GraphInstanceForm } from '#/api/operator/graph/model';
...
import { Modal, Popconfirm, Space, Progress, message } from 'ant-design-vue';
...
import { columns, querySchema, renderStatusTag, getStatusString } from './data';
...
async function handleBuild(row: Required<GraphInstanceForm>) {
  ...
  try {
    ...
    console.log('📈 当前 buildingInstances:', Array.from(buildingInstances.value));
...
async function handleViewGraph(row: Required<GraphInstanceForm>) {
  ...
  const graphRoutes = routes.filter(r => 
    r.path.includes('graph') || r.path.includes('Graph')
  );
  console.log('4. 图谱相关路由:', graphRoutes.map(r => ({
    path: r.path,
    name: r.name,
  })));
  
  const targetRoute = routes.find(r => r.path === '/operate/graph/graphVisualization');
...
function startPollingStatus(instanceId: string) {
  ...
  const timer = setInterval(async () => {
    try {
      ...
      console.log('📈 当前 buildingInstances:', Array.from(buildingInstances.value));
      console.log('📊 当前 buildProgress:', Array.from(buildProgress.value.entries()));
    ...
    <BasicTable table-title="图谱实例列表">
      ...
      <template #action="{ row }">
        <Space>
          ...
          <ghost-button
            v-if="['NOT_BUILT', 'FAILED'].includes(getStatusString(row.graphStatus))"
