...
import { graphInstanceAdd, graphInstanceInfo, graphInstanceUpdate } from '#/api/operator/graph';
  ...
  onOpenChange: async (isOpen) => {
    ...
    if (isUpdate.value && id) {
      ...
      const formData: any = {
        ...
        relationTypes: record.relationTypes ? record.relationTypes.split(',') : [],
      };
...
async function handleConfirm() {
  try {
    ...
    const data: any = {
      ...values,
      entityTypes: Array.isArray(values.entityTypes) ? values.entityTypes.join(',') : values.entityTypes,
      relationTypes: Array.isArray(values.relationTypes) ? values.relationTypes.join(',') : values.relationTypes,
    };
