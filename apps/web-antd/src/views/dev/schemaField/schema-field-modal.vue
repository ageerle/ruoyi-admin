    ...
    <Form :label-col="{ span: 6 }">
      <FormItem label="数据模型" v-bind="validateInfos.schemaId">
        <Select v-model:value="formData.schemaId" :options="props.schemaOptions"
          :field-names="{ label: 'name', value: 'id' }" :get-popup-container="getPopupContainer"
      ...
      <FormItem label="字段类型" v-bind="validateInfos.type">
        <Select v-model:value="formData.type" :options="typeOptions" :get-popup-container="getPopupContainer"
          :placeholder="$t('ui.formRules.required')" />
      </FormItem>
      <FormItem label="字段注释">
      ...
      <FormItem label="字段长度">
        <InputNumber v-model:value="formData.length" :min="0" placeholder="请输入字段长度" class="w-full" />
      </FormItem>
      <FormItem label="小数位数">
        <InputNumber v-model:value="formData.scale" :min="0" placeholder="请输入小数位数" class="w-full" />
      </FormItem>
      <FormItem label="默认值">
        <Input v-model:value="formData.defaultValue" placeholder="请输入默认值" />
      </FormItem>
      <FormItem label="排序">
        <InputNumber v-model:value="formData.sort" :min="0" placeholder="请输入排序" class="w-full" />
      </FormItem>
      <FormItem label="是否主键">
      ...
      <FormItem label="查询方式">
        <Select v-model:value="formData.queryType" :options="queryTypeOptions" :get-popup-container="getPopupContainer"
          placeholder="请选择查询方式" />
      </FormItem>
      <FormItem label="显示类型">
        <Select v-model:value="formData.htmlType" :options="htmlTypeOptions" :get-popup-container="getPopupContainer"
          placeholder="请选择显示类型" />
      </FormItem>
      <FormItem label="字典类型">
